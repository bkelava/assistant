...
from .gfi_frame import GFIFrame
...
class Menu(ProgramFrame):
    ...
    def __set_up_ui(self) -> None:
        ...
        self._frames.append(self._working_hours_sheet)
        self._GFI_workspace: GFIFrame = None
        self._frames.append(self._GFI_workspace)
        ...
        self._button_GFI: ctk.CTkButton = ctk.CTkButton(
            self,
            text=Btn.GFI,
            font=button_font,
            command=self.__create_GFI_workspace,
            fg_color=Color.BLUE_LIGHT_1529,
            text_color=Color.BLUE_1529,
            hover_color=Color.GREY_1529,
        )
        self._button_GFI.grid(padx=15, pady=15, column=0, row=14, rowspan=2, sticky=ctk.NSEW)
        self._buttons[nameof(self._button_GFI)] = self._button_GFI

    def __exit(self) -> None:
        self._controller.master.destroy()

    def __create_GFI_workspace(self) -> None:
        self.__set_up_workspace(nameof(self._button_GFI))
        self._GFI_workspace = GFIFrame(self._controller.frame_workspace)
        self._GFI_workspace.grid(padx=5, pady=5, column=0, row=0, columnspan=20, rowspan=20, sticky=ctk.NSEW)
