...
from src.constants.specials import DOWN_DASH, EMPTY_STRING, SPACE
from src.tables import Employee, PERSONAL_ID

NAME_AND_LAST_NAME: str = "name_and_lastname"
NKD_FILE_PATH: str = "src/assets/files/nkd.csv"
...
def get_nkd_from_nkd_number(nkd: str) -> str:
    data: Dict = {}
    with open(NKD_FILE_PATH, encoding="utf-8") as csvf:
        for row in csvf:
            data[row.split(";")[0]] = row.split(";", maxsplit=1)[1][:-2]
    return data[nkd]
